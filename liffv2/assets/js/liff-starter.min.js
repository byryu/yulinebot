...
*/
window.onload = function() { initVConsole(), initContent(), initLiff() };
var HttpClient = function() {
    this.get = function(e, t) {
        var a = new XMLHttpRequest;
        a.onreadystatechange = function() { 4 == a.readyState && 200 == a.status && t(a.responseText) }, a.open("GET", e, !0), a.send(null)
    }
};

function initVConsole() { window.vConsole = new window.VConsole({ defaultPlugins: ["system", "network", "element", "storage"], maxLogNumber: 1e3, onReady: function() { console.log("vConsole is ready.") }, onClearLog: function() { console.log("on clearLog") } }) }
...
function initLiff() {
    console.log("going to initialize LIFF"), liff.init(e => {
        ...
        liff.getProfile().then(e => {
            ...
            console.info("User name is", t), "yes" == getParameterByName("auto") && sendLiffMessage(), document.getElementById("greet").innerHTML = "妳好 " + t + " 感謝您的使用"
        }).catch(e => { console.error("LIFF getProfile failed", e) })
    }, e => { console.error("LIFF initialization failed", e) })
}
...
function sendLiffMessage() {
    ...
    liff.getProfile().then(a => {
        ...
        if ("profile" == e) liff.sendMessages([{ type: "flex", altText: "Profile " + n, contents: { type: "bubble", hero: { type: "image", url: a.pictureUrl, size: "full", aspectRatio: "1:1", aspectMode: "cover", action: { type: "uri", uri: "line://app/2007166301-yJJ4zwLd?auto=yes&type=image&downloadUrl=" + a.pictureUrl + "&previewUrl=" + a.pictureUrl } }, body: { type: "box", layout: "vertical", contents: [{ type: "text", text: n, align: "center", weight: "bold", size: "xl" }, { type: "box", layout: "vertical", margin: "lg", spacing: "sm", contents: [{ type: "text", text: a.statusMessage, wrap: !0, color: "#666666", size: "sm", flex: 5 }] }] }, footer: { type: "box", layout: "horizontal", spacing: "sm", contents: [{ type: "button", style: "primary", height: "sm", color: "#02afff", action: { type: "uri", label: "Square", uri: "https://line.me/ti/g2/JGUODBE4RE" }, flex: 1 }, { type: "button", style: "primary", height: "sm", action: { type: "uri", label: "Profile", uri: "line://app/2007166301-yJJ4zwLd?type=profile" }, flex: 2 }, { type: "spacer", size: "sm" }], flex: 0 } } }]).then(() => { console.log("Success sending message"), liff.closeWindow() }).catch(e => { console.error("Sending message failed", e) });
        else if ("text" == e) console.log("Start sending message"), liff.sendMessages([{ type: "text", text: document.getElementById("text").value }]).then(() => { console.log("Success sending message"), liff.closeWindow() }).catch(e => { console.error("Sending message failed", e) });
        else if ("sticker" == e) console.log("Start sending message"), liff.sendMessages([{ type: "sticker", packageId: document.getElementById("packageId").value, stickerId: document.getElementById("stickerId").value }]).then(() => { console.log("Success sending message"), liff.closeWindow() }).catch(e => { console.error("Sending message failed", e) });
        else if ("stickerimage" == e) console.log("Start sending message"), stickerId = document.getElementById("stickerId").value, packageId = document.getElementById("packageId").value, animation = document.getElementById("animation").checked, 1 == animation ? imageUrl = "https://stickershop.line-scdn.net/stickershop/v1/sticker/" + stickerId + "/IOS/sticker_animation@2x.png" : imageUrl = "https://stickershop.line-scdn.net/stickershop/v1/sticker/" + stickerId + "/IOS/sticker@2x.png", liff.sendMessages([{ type: "template", altText: n + " sent a sticker.", template: { type: "image_carousel", columns: [{ imageUrl: imageUrl, action: { type: "uri", uri: "line://shop/sticker/detail/" + packageId } }] } }]).then(() => { console.log("Success sending message"), liff.closeWindow() }).catch(e => { console.error("Sending message failed", e) });
        else if ("image" == e) console.log("Start sending message"), liff.sendMessages([{ type: "image", originalContentUrl: document.getElementById("downloadUrl").value, previewImageUrl: document.getElementById("previewUrl").value }]).then(() => { console.log("Success sending message"), liff.closeWindow() }).catch(e => { console.error("Sending message failed", e) });
        else if ("video" == e) console.log("Start sending message"), liff.sendMessages([{ type: "video", originalContentUrl: document.getElementById("downloadUrl").value, previewImageUrl: document.getElementById("previewUrl").value }]).then(() => { console.log("Success sending message"), liff.closeWindow() }).catch(e => { console.error("Sending message failed", e) });
        else if ("audio" == e) console.log("Start sending message"), liff.sendMessages([{ type: "audio", originalContentUrl: document.getElementById("downloadUrl").value, duration: 6e4 }]).then(() => { console.log("Success sending message"), liff.closeWindow() }).catch(e => { console.error("Sending message failed", e) });
        else if ("messages" == e) {
            console.log("Start sending message");
            var l = JSON.parse(document.getElementById("messages").value);
            liff.sendMessages(l).then(() => { console.log("Success sending message"), liff.closeWindow() }).catch(e => { console.error("Sending message failed", e) })
        } else if ("messagesUrl" == e) {
            console.log("Start sending message");
            var o = document.getElementById("messagesUrl").value;
            t.get(o, function(e) {
                var t = JSON.parse(e);
                liff.sendMessages(t).then(() => { console.log("Success sending message"), liff.closeWindow() }).catch(e => { console.error("Sending message failed", e) })
            }).catch(e => { console.error("Parsing messages failed", e) })
        }
    }).catch(e => { console.error("LIFF getProfile failed", e) })
}
...
function removeElements(e) { for (var t = document.getElementsByClassName(e), a = t.length - 1; 0 <= a; a--) t[a] && t[a].parentElement && t[a].id && "data" == t[a].id && t[a].parentElement.removeChild(t[a]) }